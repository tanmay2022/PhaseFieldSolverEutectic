...
An interesting result is observed, when the simulation is run by changing the equilibrium compositions to $c^{eut}_{\alpha} =$ 0.41 and $c^{eut}_{\beta} =$ 0.71 for 70\% $\alpha$ and 30\% $\beta$ phase. At steady-state, the volume fractions become 70\% $\alpha$ and 30\% $\beta$ despite starting off with equal volume fractions of $\alpha$ and $\beta$, which is shown in Figure 7.

```{r fourth, echo=FALSE, out.width='50%', fig.align = 'center', fig.cap = "Figure 7: Variation of alpha phase-field at steady-state for 70-30 equilibrium composition."}
knitr::include_graphics('alpha7030.png')
```
...
***Solution domain*** The domain is considered to be three-dimensional in this problem. The cubic domain consists of three phases with random $\alpha$ and $\beta$ phase distribution perpendicular to the growth direction as shown in Figure 8.

```{r geoml3, echo=FALSE, out.width='50%', fig.align = 'center', fig.cap = "Figure 8: Three-dimensional geometry of the lamellar morphology."}
...
```{r blocklam, echo=FALSE, out.width='50%', fig.align = 'center', fig.cap = "Figure 9: Generated 3D mesh for lamellar morphology case."}
...
Similar to section 1.2.3, a non-uniform initial condition is specified for the phase parameters. The solid-liquid interface is set at 64 units in the growth direction from the origin. The $\alpha$ and $\beta$ phases are distributed randomly in the plane perpendicular to the growth direction.
...
```{r firstb, echo=FALSE, out.width='50%', fig.align = 'center', fig.cap = "Figure 10: Lamellar morphology at solid-liquid interface at steady-state."}
knitr::include_graphics('geoml3.png')
```
...
***Solution domain*** The domain is considered to be three-dimensional in this problem. The cuboid shaped domain consists of three phases with rod shaped $\beta$ phase parallel to the growth direction embedded within $\alpha$ phase as shown in Figure 11.

```{r geomr3, echo=FALSE, out.width='50%', fig.align = 'center', fig.cap = "Figure 11: Three-dimensional geometry of the rod morphology."}
...
```{r blockrod, echo=FALSE, out.width='50%', fig.align = 'center', fig.cap = "Figure 12: Generated 3D mesh for rod morphology case."}
...
Similar to section 1.4, the results reconstructed to time directories can be viewed by creating and opening the *rod3D.foam* case module using ParaView. The $\alpha$-$\beta$ surface area becomes minimized while maintaining equal volume fractions. The phase-field profile of rod morphology at steady-state is shown in Figure 13.

```{r firstc, echo=FALSE, out.width='50%', fig.align = 'center', fig.cap = "Figure 13: Rod morphology at solid-liquid interface at steady-state."}
knitr::include_graphics('geomr3.png')
```
