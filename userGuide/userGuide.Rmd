...
OpenFOAM solves the case in three dimensions by default but can be instructed to solve in two dimensions. Here, the mesh must be 1 cell layer thick. The domain consists of a rectangle of length in the growth direction (x), 3$D/v$ and in the normal direction (y), 1.5$D/v$. The solid-liquid interface is set at 1.2$D/v$ from the origin. A domain of size 300 in the growth direction, if resolved in 300 grid points, is said to have a dx of 1. Large variations in fields can be expected near the interfaces, so the mesh will be graded to be smaller in this region. The block structure is shown in Figure 2.
...
The end time can be considered to be the time taken for steady state diffusion of each phase, which is found to be 60 for this case. To specify this end time, the stopAt keyword is set to endTime and then the endTime keyword to 60.

Next, the time step must be fixed which is represented by the keyword deltaT. To achieve temporal accuracy and numerical stability while reducing computational effort, deltaT is set to 0.05.

As the simulation progresses, results written at certain intervals of time can later be viewed with a post-processing package. The writeControl keyword can be set to the runTime option, where the time interval for writing is specified under the writeInterval keyword. The writeInterval can be set to 5. OpenFOAM creates a new directory named after the current time, e.g. 10, on each occasion that it writes a set of data containing the results for each field, $\phi_i$ and $\mu$, into the time directories. For this case, the entries in the *controlDict* are shown below:
...
```
The *fvSolution* dictionary contains sub-dictionaries: SIMPLE that contains a control parameter nNonOrthogonalCorrectors set to 0, and Tol_is_defined_here that contains a control parameter Tol set to 1 (which is read by *createTol.H*). The description of other options can be found in the OpenFOAM userguide.
...
Lamellar morphology with equal volume fractions of $\alpha$ and $\beta$ phases

# 3D rod eutectic growth

Rod morphology with volume fractions of 70\% $\alpha$ and 30\% $\beta$ phase
